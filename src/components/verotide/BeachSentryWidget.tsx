    ...
    <div className="terminal-box p-6 flex flex-col gap-4 border-primary/20 rounded-xl transition-all hover:border-primary/40 group">
      ...
      {error ? (
        ...
        <div className="grid grid-cols-2 gap-4 mt-2">
          ...
          <div className="p-3 border-2 border-primary/10 bg-black/60 rounded-md group/redtide relative cursor-help">
            <div className="text-[10px] text-primary/40 font-black uppercase mb-1 tracking-widest text-center">Red_Tide</div>
            <div className="text-xs font-black text-primary uppercase text-center tracking-[0.2em]">CLEARED</div>

            {/* Hover Tooltip */}
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-48 bg-primary text-black text-[10px] font-black p-2 rounded opacity-0 group-hover/redtide:opacity-100 transition-opacity pointer-events-none z-50 shadow-2xl uppercase tracking-tighter text-center">
              FWC monitoring active. Zero harmful algal blooms detected.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="text-xs bg-yellow-400/10 p-3 border-l-4 border-yellow-400 mt-2 font-mono flex justify-between items-center shadow-lg rounded-r-md group/turtle relative cursor-help">
        ...
        <span className="text-yellow-400 font-black uppercase animate-pulse tracking-tighter shadow-yellow-400/20">Active_Season</span>
        ...
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-64 bg-primary text-black text-[10px] font-black p-2 rounded opacity-0 group-hover/turtle:opacity-100 transition-opacity pointer-events-none z-50 shadow-2xl uppercase tracking-tighter text-center">
          Sea Turtle protection protocols active. Minimize beach illumination from 9PM-7AM.
        </div>
      </div>
    </div>
  );
